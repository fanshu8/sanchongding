    ...
    <EducationPageTemplate content={content} language={lang}>
          ...
          <div className="p-6 border-2 border-black dark:border-white">
            ...
            <div className="space-y-3 text-gray-700 dark:text-gray-300">
              ...
              <p className="text-sm">
                ...
                {isZh
                  ? '1) Star形态的跳空在数字货三重市场不常见（24小时交易），重点看实体大小对比；2) 第3根K线必须"深入"第1根K线实体（至少吞没50%）；3) 出现频率低但可靠性高，值得耐心等待；4) 在D1图表最有效，H4其次，H1以下不建议使用。'
                  : '1) Star pattern gaps uncommon in cryptocurrency (24hr trading), focus on body size contrast; 2) 3rd candle must "penetrate" 1st candle body (at least 50% engulfing); 3) Low frequency but high reliability, worth patient wait; 4) Most effective on D1 chart, H4 next, not recommended below H1.'}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="p-6 bg-black dark:bg-white text-white dark:text-black">
            ...
            <ul className="space-y-3">
              ...
              <li>
                ...
                {isZh
                  ? '数字货三重现货交易没有真实成交量，但期货成交量或Tick Volume可作为参考。形态伴随量增（看涨形态）或量减（看跌形态）时更可靠。非必需条件。'
                  : 'Cryptocurrency spot has no true volume, but futures volume or Tick Volume can reference. Pattern with volume increase (bullish) or decrease (bearish) more reliable. Not essential condition.'}
              </li>
              <li>
      ...
      <section id="practical-cases" className="mb-16 scroll-mt-20">
        ...
        <div className="space-y-6">
          <div className="p-6 border-2 border-green-600 bg-green-50 dark:bg-green-900/20">
            <h3 className="text-lg font-bold mb-3 text-black dark:text-white">
              {isZh ? '案例1：BTC/USDT完美看涨吞没（H4图）' : 'Case 1: BTC/USDT Perfect Bullish Engulfing (H4 Chart)'}
            </h3>
            <div className="space-y-2 text-gray-700 dark:text-gray-300 text-sm">
              <p>
                <strong>{isZh ? '市场背景：' : 'Market Context:'}</strong>{' '}
                {isZh
                  ? '2024年1月18日，BTC/USDT在H4图表经历3天下跌（从1.0950跌至1.0850），触及1.0850关键支撑位（前低+整数关口）。'
                  : 'Jan 18, 2024, BTC/USDT on H4 chart experienced 3-day decline (1.0950 to 1.0850), touched 1.0850 key support (previous low + round number).'}
              </p>
              <p>
          ...
          <div className="p-6 border-2 border-green-600 bg-green-50 dark:bg-green-900/20">
            <h3 className="text-lg font-bold mb-3 text-black dark:text-white">
              {isZh ? '案例2：ETH/USDT Pin Bar反转（H1图）' : 'Case 2: ETH/USDT Pin Bar Reversal (H1 Chart)'}
            </h3>
            <div className="space-y-2 text-gray-700 dark:text-gray-300 text-sm">
              <p>
                <strong>{isZh ? '市场背景：' : 'Market Context:'}</strong>{' '}
                {isZh
                  ? '2024年1月20日，ETH/USDT在H1图表上涨至1.2750阻力位（前高），随后出现看跌Pin Bar（Shooting Star）。'
                  : 'Jan 20, 2024, ETH/USDT on H1 chart rose to 1.2750 resistance (previous high), then bearish Pin Bar (Shooting Star) appeared.'}
              </p>
              <p>
                <strong>{isZh ? '形态分析：' : 'Pattern Analysis:'}</strong>
              </p>
